...
var clueArr = []
var clueTxtArr = []
var cnt = -1, qscnt = -1, ans, uans, interval, delayInterval, time = 18, totalQuestions = 10, answeredQuestions = 0, choiceCnt = 12, quesCnt = 0, resTimerOut = 0, rst = 0, responseTime = 0, correctAnswer = "", lCnt = -1, wrdCnt = -1;
...
var isScale = true
var scaleType = 1;
var words_arry = ["apple", "banana", "cherry", "grapes", "mango", "orange", "pomegranate", "strawberry", "avocado", "watermelon",
                    "pear","lemon","kiwifruit","papaya","apricot","pineapple","lychee","dragonfruit","fig","jackfruit"];
...
function doneLoading1(event) {
    ...
    stage.update();
		call_UI_gameQuestion(container,"Click on the correct letter to complete the name of the fruit");

    if (id == "question") {
        var choiceSpriteSheet = new createjs.SpriteSheet({
            framerate: 30,
            "images": [preload.getResult("question")],
            "frames": { "regX": 50, "height": 298, "count": 64, "regY": 50, "width": 298 },
...
function CreateGameElements() {
    ...
    if (isQuestionAllVariations) {
        ...
        chposArr = [0, 1, 2, 0, 1, 0, 2, 1, 0, 2,0, 1, 2, 0, 1, 0, 2, 1, 0, 2]
        // pickques()
    } else {
...
function pickques() {
   ...
   correctAnswer = words_arry[qno[cnt]];
    ...
    for (i = 0; i < cLen; i++) {
        ...
        if (cLen == 4) {
            quesArr[i].x = 400 + (i * 150);
        }
        if (cLen == 5) {
            quesArr[i].x = 387 + (i * 120);
        }
        if (cLen == 6) {
            quesArr[i].x = 318 + (i * 120);
        }
        if (cLen == 7) {
            quesArr[i].x = 265 + (i * 120);
        }
        if (cLen == 8) {
            quesArr[i].x = 205 + (i * 120);
        }
