...
var startBtn, introScrn, container, question, circleOutline, chHolderMC, choice1, choice2, choice3, helpMc, backGround1, kholderMc, ansPanelMc, clueMc, clueMc1, resultLoading, selectedAnswer = "", cLen = 0;
...
var words_arry = ["clock","basket","sofa","scissors","chair","lamp","fridge","kettle","key","utensils"];
var maxLetterCnt = 10
var quesArr = []
...
function init() {
    ...
    assetsPath = "assets/";
    gameAssetsPath = "WhoAmI-HouseHoldThings/";
    soundpath = "FA/"
...
function doneLoading1(event) {
    ...
    stage.update();
	call_UI_gameQuestion(container,"Click on the correct letter to complete the name of the household object");
    if (id == "question") {
        var choiceSpriteSheet = new createjs.SpriteSheet({
            framerate: 30,
            "images": [preload.getResult("question")],
            "frames": { "regX": 50, "height": 265, "count": 64, "regY": 50, "width": 265 },
    ...
    if (id == "question1") {
        var choiceSpriteSheet = new createjs.SpriteSheet({
            ...
            "frames": { "regX": 50, "height": 115, "count": 64, "regY": 50, "width": 112 },
            // define two animations, run (loops, 1.5x speed) and jump (returns to run):
        });
...
console.log("clen"+cLen)
        ...
        if (cLen == 5) {
            quesArr[i].x = 393 + (i * 120);
        }
        if (cLen == 6) {
            quesArr[i].x = 334 + (i * 120);
        }
        if (cLen == 7) {
            quesArr[i].x = 265 + (i * 120);
        }
        if (cLen == 8) {
            quesArr[i].x = 212 + (i * 120);
        }
...
function createTween() {
 ...
 .to({  y:260, scaleX:.75,scaleY :.75,alpha: 1 }, 250)
        .to({  y:260, scaleX:.8,scaleY :.8,alpha: 1 }, 250).call(() => { 
    try { 
	...
	{
		createjs.Tween.get(question).wait(500)
 .to({  y:266, scaleX:.75,scaleY :.75,alpha: 1 }, 250)
        .to({  y:266, scaleX:.8,scaleY :.8,alpha: 1 }, 250).call(() => { 
    try { 
      ChoiceFX_entrance(choiceArr, 1600); 
    } catch (_e) {} 
  }).to({ scaleX:.8,scaleY :.8,alpha: 1 }, 250)
	...
	{
		createjs.Tween.get(question).wait(500)
 .to({  y:269, scaleX:.75,scaleY :.75,alpha: 1 }, 250)
        .to({  y:269, scaleX:.8,scaleY :.8,alpha: 1 }, 250).call(() => { 
    try { 
      ChoiceFX_entrance(choiceArr, 1600); 
    } catch (_e) {} 
  }).to({ scaleX:.8,scaleY :.8,alpha: 1 }, 250)
	}
	else if(qno[cnt] == 0 || qno[cnt] == 1)
	{
		console.log('csdd')
		createjs.Tween.get(question).wait(500)
 .to({  y:270, scaleX:.75,scaleY :.75,alpha: 1 }, 250)
        .to({  y:270, scaleX:.8,scaleY :.8,alpha: 1 }, 250).call(() => { 
    try { 
      ChoiceFX_entrance(choiceArr, 1600); 
    } catch (_e) {} 
  }).to({ scaleX:.8,scaleY :.8,alpha: 1 }, 250)
		...
		createjs.Tween.get(question).wait(500)
 .to({  y:262, scaleX:.75,scaleY :.75,alpha: 1 }, 250)
        .to({  y:262, scaleX:.8,scaleY :.8,alpha: 1 }, 250).call(() => { 
    try { 
      ChoiceFX_entrance(choiceArr, 1600); 
    } catch (_e) {} 
  }).to({ scaleX:.8,scaleY :.8,alpha: 1 }, 250)
		...
		createjs.Tween.get(question).wait(500)
 .to({  y:274, scaleX:.75,scaleY :.75,alpha: 1 }, 250)
        .to({  y:274, scaleX:.8,scaleY :.8,alpha: 1 }, 250).call(() => { 
    try { 
      ChoiceFX_entrance(choiceArr, 1600); 
    } catch (_e) {} 
  }).to({ scaleX:.8,scaleY :.8,alpha: 1 }, 250)
 ...
 .to({  y:266, scaleX:.75,scaleY :.75,alpha: 1 }, 250)
    ...
    try { 
      ChoiceFX_entrance(choiceArr, 1600); 
		...
		createjs.Tween.get(question).wait(500)
 .to({  y:257, scaleX:.75,scaleY :.75,alpha: 1 }, 250)
        .to({  y:257, scaleX:.8,scaleY :.8,alpha: 1 }, 250).call(() => { 
    try { 
      ChoiceFX_entrance(choiceArr, 1600); 
    } catch (_e) {} 
  }).to({ scaleX:.8,scaleY :.8,alpha: 1 }, 250)
...
function answerSelected(e) {
	try { ChoiceFX_pressRipple(container, e.x, e.y); } catch(_e){}
    e.preventDefault();
    lCnt++;
