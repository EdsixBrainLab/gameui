...
function CreateGameElements() {
    ...
    if (questionText && !questionText.parent) {
        container.parent.addChild(questionText);
    }
    if (questionText) {
        questionText.visible = false;
    ...
    question1.x = 610
    question1.y = 270
    question1.scaleX = question1.scaleY = 1.2
...
function createTween() {
    ...
    clearquesInterval = setInterval(createChoices, 5200);
}
...
function createTween1() {
    ...
    if (qtype[cnt] == 1) {
        ...
        animateChoiceOptions(choice1Arr, 1.2, 360, AddListenerFn);
    } else {
        question.visible = true;
        question1.visible = false;
        animateQuestionPrompt(question, 1.5, 200);
        animateChoiceOptions(choice2Arr, 1.2, 360, AddListenerFn);
    }
}
...
function animateQuestionPrompt(target, baseScale, delay) {
    ...
    createjs.Tween.get(target, { override: true })
        .wait(delay || 0)
        .to({ alpha: 1, scaleX: scale, scaleY: scale }, 420, createjs.Ease.backOut);
}

function animateChoiceOptions(arr, baseScale, revealDelay, onComplete) {
    if (!arr) { return; }
    var hasTweens = false;
    var pendingTweens = 0;
    for (var idx = 0; idx < arr.length; idx++) {
        ...
        var targetX = tile.__targetX || tile.x;
        var targetY = tile.__targetY || tile.y;
        tile.visible = true;
        tile.alpha = 0;
        tile.x = targetX;
        tile.y = targetY + 60;
        ...
        (function (target, index, finalX, finalY, base) {
            var delay = (revealDelay || 0) + (index * 150);
            createjs.Tween.get(target, { override: true })
                .wait(delay)
...
function handleChoiceOver(e) {
    ...
    if (!target) { return; }
    var baseScale = target.baseScale || 1.2;
