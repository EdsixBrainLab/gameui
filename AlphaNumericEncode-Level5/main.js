...
function createTween1() {
    ...
    if (qtype[cnt] == 1) {
        ...
        animateChoiceOptions(choice1Arr, 1.24, 360, AddListenerFn);
    } else {
        question.visible = true;
        question1.visible = false;
        animateQuestionPrompt(question, 1.5, 200);
        animateChoiceOptions(choice2Arr, 1.24, 360, AddListenerFn);
    }
}
...
function animateChoiceOptions(options, baseScale, revealDelay, onComplete) {
...
function handleChoiceOver(e) {
    ...
    if (!target) { return; }
    var baseScale = target.baseScale || 1.24;
