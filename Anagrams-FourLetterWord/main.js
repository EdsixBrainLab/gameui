...
var WORD_LENGTH = 4;
var WORDS_PER_ROUND = 3;
var TOTAL_CLUE_SLOTS = WORD_LENGTH * WORDS_PER_ROUND;

var currentAnswerSet = [];
var foundAnswersMap = {};
var currentWordLetters = [];
var currentChoiceIndices = [];
var filledClueSlots = 0;
...
var maxLetterCnt = TOTAL_CLUE_SLOTS;
...
loaderBar.visible = false;
  ...
  call_UI_gameQuestion(
    container,
    "Make three four-letter words using these letters"
  );
  ...
  if (id == "clueMc") {
    var clueSpriteSheet = new createjs.SpriteSheet({
      ...
      frames: { regX: 34, height: 60, count: 0, regY: 30, width: 67 },
      // define two animations, run (loops, 1.5x speed) and jump (returns to run):
    });
...
function pickques() {
  ...
  isCorrect = "";
  currentAnswerSet = multiAnswerMap[qno[cnt]] || [words_arry[qno[cnt]]];
  correctAnswer =
    currentAnswerSet && currentAnswerSet.length > 0
      ? currentAnswerSet[0]
      : words_arry[qno[cnt]];
  foundAnswersMap = {};
  currentWordLetters = [];
  ...
  for (var row = 0; row < WORDS_PER_ROUND; row++) {
    ...
    for (var col = 0; col < WORD_LENGTH; col++) {
      ...
      if (clueArr[slotIndex]) {
        ...
        clueArr[slotIndex].alpha = 0;
        clueArr[slotIndex].x = slotX;
        clueArr[slotIndex].y = slotY + CLUE_LETTER_VERTICAL_OFFSET;
        clueArr[slotIndex].scaleX = clueArr[slotIndex].scaleY = slotScale;
      }

      if (clueBgArr[slotIndex]) {
        drawClueSlotBackground(clueBgArr[slotIndex]);
        clueBgArr[slotIndex].x = slotX;
        clueBgArr[slotIndex].y = slotY;
        clueBgArr[slotIndex].scaleX = clueBgArr[slotIndex].scaleY = slotScale;
        clueBgArr[slotIndex].visible = true;
        clueBgArr[slotIndex].alpha = 0;
        clueBgArr[slotIndex].__baseScale = slotScale;
      }
    }
  }
...
function answerSelected(e) {
  ...
  if (filledClueSlots >= TOTAL_CLUE_SLOTS || currentWordLetters.length >= WORD_LENGTH) {
    return;
  }
  ...
  var letterValue = e.currentTarget.name || "";
  var upperLetter = String(letterValue).toUpperCase();
  if (!upperLetter) {
    return;
  }

  if (choiceMcArr[selectedIndex]) {
    choiceMcArr[selectedIndex].mouseEnabled = false;
    choiceMcArr[selectedIndex].cursor = "default";
  }
  ...
  if (choiceArr[selectedIndex]) {
    choiceArr[selectedIndex].alpha = 0.6;
  }

  strArr.push(upperLetter);

  var slotIndex = filledClueSlots;
  if (clueArr[slotIndex]) {
    updateClueLetterDisplay(clueArr[slotIndex], upperLetter);
    clueArr[slotIndex].visible = true;
  }

  filledClueSlots = Math.min(filledClueSlots + 1, TOTAL_CLUE_SLOTS);

  currentWordLetters.push(upperLetter.toLowerCase());
  currentChoiceIndices.push(selectedIndex);

  if (currentWordLetters.length === WORD_LENGTH) {
    finalizeCurrentWordSelection();
  }
}

function finalizeCurrentWordSelection() {
  gameResponseTimerStop();

  for (var idx = 0; idx < cLen; idx++) {
    if (choiceMcArr[idx]) {
      choiceMcArr[idx].mouseEnabled = false;
      choiceMcArr[idx].cursor = "default";
    }
    detachChoiceInteractions(idx);
    stopChoiceIdleAnimation(idx);
    if (typeof SA_setChoiceInteractiveState === "function") {
      SA_setChoiceInteractiveState(idx, false, { immediate: true, suppressBadge: true });
    }
  }

  var candidate = currentWordLetters.join("");
  var normalizedCandidate = candidate.toLowerCase();
  var slotStartIndex = Math.max(0, filledClueSlots - WORD_LENGTH);

  var isCorrectWord =
    currentAnswerSet.indexOf(normalizedCandidate) !== -1 &&
    !foundAnswersMap[normalizedCandidate];

  for (var i = 0; i < currentChoiceIndices.length; i++) {
    var choiceIndex = currentChoiceIndices[i];
    markChoiceResult(choiceIndex, isCorrectWord);
  }

  for (var j = 0; j < WORD_LENGTH; j++) {
    animateClueSlotFill(slotStartIndex + j, isCorrectWord);
  }

  if (isCorrectWord) {
    foundAnswersMap[normalizedCandidate] = true;

    if (Object.keys(foundAnswersMap).length === WORDS_PER_ROUND) {
      getValidation("correct");
      disablechoices();
    } else {
      setTimeout(function () {
        resetChoicesForNextWord();
        gameResponseTimerStart();
      }, 800);
    }
  } else {
...
function resetChoicesForNextWord() {
  currentWordLetters = [];
  currentChoiceIndices = [];
