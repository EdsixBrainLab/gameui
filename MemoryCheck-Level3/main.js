...
function init() {
    ...
    assetsPath = "assets/";
    gameAssetsPath = "MemoryCheck-Level3/";
    soundpath = "FA/"
    ...
    if (success == 1) {
        manifest.push(
            { id: "choice1", src: gameAssetsPath + "ChoiceImages1.png" },
             { id: "questionText", src: questionTextPath + "MemoryCheck-Level3-QT.png" },
...
function doneLoading1(event) {
    ...
    if (id == "choice1") {
        var spriteSheet1 = new createjs.SpriteSheet({
            ...
            "frames": { "regX": 0, "height": 281, "count": 0, "regY": 0, "width": 282 }
        });
...
function pickques() {
    ...
    btnx = [220, 770, 220, 770]
    btny = [170, 170, 440, 440]
...
function createTween() {
    ...
    createjs.Tween.get(choiceArr[2]).wait(600).to({ alpha: 0, scaleX: .95, scaleY: .95 }, 500).to({ alpha: 1, scaleX: 1, scaleY: 1 }, 500)
...
function createChoices() {
    ...
    for (i = 0; i < 4; i++) {
        ...
        switch (i) {
            case 0:
            choiceArr1[chpos[i]].x = 220;
            choiceArr1[chpos[i]].y = 170;

                break;
            case 1:
            choiceArr1[chpos[i]].x = 820;
            choiceArr1[chpos[i]].y = 170;
                break;

            case 2:
            choiceArr1[chpos[i]].x = 220;
            choiceArr1[chpos[i]].y = 440;
                break;
            case 3:
            choiceArr1[chpos[i]].x = 820;
            choiceArr1[chpos[i]].y = 440;
...
function createTween1() {
    ...
    createjs.Tween.get(choiceArr1[0]).to({ alpha: 0, scaleX: .8, scaleY: .8 }, 200).to({alpha:0.5,rotation:180},200)
        .to({ visible: true, alpha: 1, scaleX:.85, scaleY:.85 }, 200,createjs.Ease.bounceIn).to({alpha:1,rotation:360},200)
        .to({ visible: true, alpha: 1, scaleX: .9, scaleY: .9 }, 200,createjs.Ease.bounceIn)
        createjs.Tween.get(choiceArr1[1]).to({ alpha: 0, scaleX: .8, scaleY: .8 }, 200).to({alpha:0.5,rotation:180},200)
        .to({ visible: true, alpha: 1, scaleX:.85, scaleY:.85 }, 200,createjs.Ease.bounceIn).to({alpha:1,rotation:360},200)
        .to({ visible: true, alpha: 1, scaleX: .9, scaleY: .9 }, 200,createjs.Ease.bounceIn)
        createjs.Tween.get(choiceArr1[2]).to({ alpha: 0, scaleX: .8, scaleY: .8 }, 200).to({alpha:0.5,rotation:180},200)
        .to({ visible: true, alpha: 1, scaleX:.85, scaleY:.85 }, 200,createjs.Ease.bounceIn).to({alpha:1,rotation:360},200)
        .to({ visible: true, alpha: 1, scaleX: .9, scaleY: .9 }, 200,createjs.Ease.bounceIn)
        createjs.Tween.get(choiceArr1[3]).to({ alpha: 0, scaleX: .8, scaleY: .8 }, 200).to({alpha:0.5,rotation:180},200)
        .to({ visible: true, alpha: 1, scaleX:.85, scaleY:.85 }, 200,createjs.Ease.bounceIn).to({alpha:1,rotation:360},200)
        .to({ visible: true, alpha: 1, scaleX: .9, scaleY: .9 }, 200,createjs.Ease.bounceIn)
