...
function init() {
    ...
    assetsPath = "assets/";
    gameAssetsPath = "SequenceMemory-Level5/";
    soundpath = "FA/"
    ...
    if (success == 1) {
        manifest.push(
            ...
            { id: "questionText", src: questionTextPath + "SequenceMemory-Level5-QT.png" },
        )
        preloadAllAssets()
        stage.update();
    }
...
function doneLoading1(event) {
    ...
    if (id == "questionText") {
        ...
        var spriteSheet1 = new createjs.SpriteSheet({
            ...
            "frames": { "regX": 50, "height": 89, "count": 0, "regY": 50, "width": 579 },
            // define two animations, run (loops, 1.5x speed) and jump (returns to run):
        });
        //	"frame": {"x":0,"y":0,"w":711,"h":135},
...
function pickques() {
    ...
    createTween()
    questionInterval = setInterval(delayStartQuestion, 6000) // two seconds 
...
function createTween() {
    ...
    for (i = 0; i < 7; i++) {
        choiceArr[i].visible = true;
        choiceArr[i].alpha = 0;
        choiceArr[i].scaleX = .80
        choiceArr[i].scaleY = .80
        createjs.Tween.get(choiceArr[i]).wait(val).to({ scaleX: .85, scaleY: .85, alpha: 1 }, val)
        val = val + 150
    }
...
function delayStartQuestion() {
    ...
    var chpos1 = between(0, 6)
    var shuffle = between(0, 6)
    var j;
