...
var assets = [];
var cnt = -1, qscnt = -1, ans, uans, interval, time = 18, totalQuestions = 10, answeredQuestions = 0, choiceCnt = 3, quesCnt = 0, resTimerOut = 0, rst = 0, responseTime = 0;
var startBtn, introScrn, container, choice1, choice2, choice3, choice4, question, ansHolder, circle1Outline, boardMc, helpMc, quesMarkMc, questionText, quesHolderMc, resultLoading, preloadMc, introHintImg, introHintImg1;
...
function init() {
    ...
    assetsPath = "assets/";
    gameAssetsPath = "MindCapture-Level1/";
    soundpath = "FA/"
    ...
    if (success == 1) {
        manifest.push(
            ...
            { id: "questionText", src: questionTextPath + "MindCapture-Level1-QT.png" },
            { id: "no", src: questionTextPath + "MindCapture-Level1-QT1.png" },
            { id: "yes", src: questionTextPath + "MindCapture-Level1-QT2.png" }
        )
        preloadAllAssets()
        stage.update();
    }
...
function CreateGameElements() {
    ...
    for (i = 1; i <= 2; i++) {
        ...
        quesArr[i].scaleX = quesArr[i].scaleY = 1.3
        quesArr[i].x = btnx[i]
        quesArr[i].y = 220
    }
    ...
    yesMc.name = "Y";
    yesMc.x = 430; yesMc.y = 630;

    noMc.name = "N";
    noMc.x = 800; noMc.y = 630;

    posChangeArr.sort(randomSort)
    if (isQuestionAllVariations) {

        posArr = [0, 1, 2, 0, 1, 0, 3, 2, 1, 3, 0, 1, 2, 0, 1, 0, 3, 2, 1, 3, 0, 1, 2, 0, 1, 0, 3, 2, 1, 3, 0, 1, 2, 0, 1, 0, 3, 2, 1, 3]
        //   createGameWiseQuestions()
        //  pickques()
    } else {
        posArr = [0, 1, 2, 0, 1, 0, 3, 2, 1, 3]
        //   pickques()
    }
...
function createTween1() {
    ...
    if (posChangeArr[cnt] == 0) {
        question.y = -300
        createjs.Tween.get(question).wait(1000).to({ alpha: 1, x: question.x, y: 250 }, 600, createjs.Ease.bounceOut);
    } else {
        question.y = 900
        createjs.Tween.get(question).wait(1000).to({ alpha: 1, x: question.x, y: 250 }, 600, createjs.Ease.bounceOut);
    }
