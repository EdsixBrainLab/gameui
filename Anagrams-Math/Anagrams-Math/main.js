...
var nameArr = ["tow", "dad", "net", "evens", "voles",
    "pinto", "there", "height", "sent", "chin",
    "cleric", "once", "relating", "covert", "items",
    "declaim", "rifts", "handouts", "phase", "angel"];

var words_arry = ["two", "add", "ten", "seven", "solve",
    "point", "three", "eighth", "tens", "inch",
    "circle", "cone", "triangle", "vector", "times",
    "decimal", "first", "thousand", "shape", "angle"];
...
function init() {
    ...
    if (success == 1) {
        manifest.push(
            ...
            { id: "chHolder", src:questionTextPath+ "Anagrams-Math-QT1.png" },
            { id: "choice1", src: gameAssetsPath + "ChoiceImages1.png" },
            { id: "QusTxtString", src:questionTextPath+ "Anagrams-Math-QT.png" },
...
function enablechoices() {
    ...
    for (i = 0; i < cLen; i++) {
        ...
        if (cLen == 2) {
            clueArr[i].x = 585 + (i * 120) //- 14;
            choiceArr[i].x = 430 + (i * 175);

        }

        if (cLen == 3) {
            clueArr[i].x = 509 + (i * 120) //- 14;
            choiceArr[i].x = 430 + (i * 175);

        }
        if (cLen == 4) {
            clueArr[i].x = 453 + (i * 120) //- 14;
            choiceArr[i].x = 337 + (i * 175);

        }
        if (cLen == 5) {
            clueArr[i].x = 392 + (i * 120) // - 14;
            choiceArr[i].x = 230 + (i * 185);

        }
        if (cLen == 6) {
            clueArr[i].x = 332 + (i * 120) //- 14;
            choiceArr[i].x = 163 + (i * 175);

        }
        if (cLen == 7) {
            clueArr[i].x = 274 + (i * 120) //- 14;
            choiceArr[i].x = 92 + (i * 170);

        }
        if (cLen == 8) {
            clueArr[i].x = 211 + (i * 120) //- 14;
            choiceArr[i].x = 75 + (i * 150);

        }

        if (cLen == 9) {
            clueArr[i].x = 393 + (i * 120) //- 14;
            choiceArr[i].x = 43 + (i * 140);
            choiceArr[i].scaleX = choiceArr[i].scaleY = .7;
            choiceArrScale = .7
            clueArr[i].scaleX = clueArr[i].scaleY = 1.1;
