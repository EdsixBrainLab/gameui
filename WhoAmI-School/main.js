...
var words_arry = ["bag", "eraser", "inkpot", "table", "chart", "blackboard", "book", "pen", "pencils", "calculator"];
var maxLetterCnt = 10
var quesArr = []
...
function doneLoading1(event) {
    ...
    stage.update();
	call_UI_gameQuestion(container,"Click on the correct letter to complete the name of the object");
    if (id == "question") {
        var choiceSpriteSheet = new createjs.SpriteSheet({
            framerate: 30,
            "images": [preload.getResult("question")],
            "frames": { "regX": 50, "height": 361, "count": 64, "regY": 50, "width": 362 },
    ...
    if (id == "question1") {
        var choiceSpriteSheet = new createjs.SpriteSheet({
            ...
            "frames": { "regX": 50, "height": 109, "count": 64, "regY": 50, "width": 113 },
            // define two animations, run (loops, 1.5x speed) and jump (returns to run):
        });
...
function pickques() {
    ...
    for (i = 0; i < cLen; i++) {
        ...
        console.log("indexArr2" + indexArr2[i]);
    }
    ...
    for (i = 0; i < cLen; i++) {
        ...
        if (cLen == 3) {
            quesArr[i].x = 484 + (i * 150);
        }
        if (cLen == 4) {
            quesArr[i].x = 411 + (i * 150);
        }
        if (cLen == 5) {
            quesArr[i].x = 393 + (i * 120);
        }
        if (cLen == 6) {
            quesArr[i].x = 336 + (i * 120);
        }
        if (cLen == 7) {
            quesArr[i].x = 274 + (i * 120);
        }
        if (cLen == 8) {
    ...
    for (i = 0; i < 3; i++) {
        ...
        if (i == 2) {
            createjs.Tween.get(choiceArr[i]).wait(val)
                .to({ y: 630, rotation: 180, scaleX: .65, scaleY: .65, alpha: .5 }, 100)
                .to({ y: 630, rotation: 360, scaleX: .7, scaleY: .7, alpha: 1 }, 500)
        }
        else {
            createjs.Tween.get(choiceArr[i]).wait(val)
                .to({ y: 630, rotation: 180, scaleX: .65, scaleY: .65, alpha: .5 }, 100)
