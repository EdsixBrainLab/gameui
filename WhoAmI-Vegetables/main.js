...
var words_arry = ["brinjal", "cabbage", "carrot", "cauliflower", "cucumber", "onion", "turnip", "tomato", "pumpkin", "potato"];
var maxLetterCnt = 10
var quesArr = []
...
function init() {
    ...
    assetsPath = "assets/";
    gameAssetsPath = "WhoAmI-Vegetables/";
    soundpath = "FA/"
    ...
    if (success == 1) {
        manifest.push(
            ...
            { id: "choice1", src: gameAssetsPath + "choiceimage.png" }
        )
        preloadAllAssets()
        stage.update();
    }
...
function doneLoading1(event) {
    ...
    stage.update();
	call_UI_gameQuestion(container,"Click on the correct letter to complete the name of the vegetable");
    if (id == "question") {
        var choiceSpriteSheet = new createjs.SpriteSheet({
            framerate: 30,
            "images": [preload.getResult("question")],
            "frames": { "regX": 50, "height": 295, "count": 64, "regY": 50, "width": 296 },
  ...
  .call(() => { 
    ...
    for (i = 0; i < 3; i++) {
        ...
        createjs.Tween.get(choiceArr[i]).wait(val)
            .to({ y: 600, rotation: 180, scaleX: .65, scaleY: .65, alpha: .5 }, 100)
            .to({ y: 600, rotation: 360, scaleX: .7, scaleY: .7, alpha: 1 }, 500)
        }
        else{
            createjs.Tween.get(choiceArr[i]).wait(val)
            .to({ y: 600, rotation: 180, scaleX: .65, scaleY: .65, alpha: .5 }, 100)
