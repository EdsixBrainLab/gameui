...
var introBalloonY = 750, introBalloonX = 570;
var introTween = []
var introquesArr = []
var introchoiceArr = []
var highlightTweenArr = []
var numCnt1 = 0
var introbtnX = [135,430,720,1010]
var vno = [5,3,6,4]

var introArrowX = 462, introArrowY = 270;
var introfingureX = 530, introfingureY = 460;
    ...
    for (i = 0; i < 4; i++) {
        ...
        introTween[i].timeline.addTween(
            createjs.Tween.get(introquesArr[i])
                .to({ x: 550, y: 50 }, 9).to({ x: this["objArr" + i][0].x + 320, y: this["objArr" + i][0].y + 5 }, timeFrameSpeed).to({ x: this["objArr" + i][1].x + 320, y: this["objArr" + i][1].y + 5 }, timeFrameSpeed).to({ x: this["objArr" + i][2].x + 320, y: this["objArr" + i][2].y + 5 }, timeFrameSpeed));
...
function createCh(){
	...
	container.parent.addChild(introQuestxt1);
    introQuestxt1.x = introQuestxtX; introQuestxt1.y = 130;
    introQuestxt1.gotoAndStop(0);
    for (i = 0; i < 4; i++) {
        ...
        if(i==3){
            createjs.Tween.get(introchoiceArr[i]).wait(100).to({ x: introbtnX[i] }, 500).wait(1000)
            .to({ alpha: 1 }).call(handleComplete2_1);
...
function introCh1() {
    introImg.y = 700
    createjs.Tween.get(introImg).to({ visible: true }).to({ alpha: 0 }).to({ alpha: 1, y: 140 }, 500).wait(2000).call(handleComplete5_1);
}
