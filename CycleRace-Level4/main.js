...
var cycleRaceImageQuestionPrompts = [
...
if (typeof window !== "undefined") {
        ...
        window.CycleRaceImageQuestionPrompts = cycleRaceImageQuestionPrompts;
}
...
function cycleRaceBuildImagePrompt(index) {
        ...
        var prompts = cycleRaceImageQuestionPrompts;
        if (prompts && prompts.length) {
                var normalizedIndex = index != null ? index : 0;
                if (normalizedIndex < 0) {
                        normalizedIndex = 0;
                }
                if (normalizedIndex >= prompts.length) {
                        normalizedIndex = prompts.length - 1;
                }

                var prompt = prompts[normalizedIndex];
                if (prompt != null && String(prompt).trim().length > 0) {
