...
function init() {
    ...
    assetsPath = "assets/";
    gameAssetsPath = "MindCapture-Level4/";
    soundpath = "FA/"
    ...
    if (success == 1) {
        manifest.push(
            ...
            { id: "questionText", src: questionTextPath + "MindCapture-Level4-QT.png" },
            { id: "no", src: questionTextPath + "MindCapture-Level4-QT1.png" },
            { id: "yes", src: questionTextPath + "MindCapture-Level4-QT2.png" }
        )
        preloadAllAssets()
        stage.update();
    }
...
function createTween1() {
    ...
    if (posChangeArr[cnt] == 0) {
        question.y = -300
        if (qch1[1] == 4 || qch1[1] == 5) {
            createjs.Tween.get(question).wait(1000).to({ alpha: 1, x: question.x, y: 392 }, 600, createjs.Ease.bounceOut);
        } else {
            createjs.Tween.get(question).wait(1000).to({ alpha: 1, x: question.x, y: 193 }, 600, createjs.Ease.bounceOut);
        }
    } else {
        question.y = 900
        if (qch1[1] == 4 || qch1[1] == 5) {
