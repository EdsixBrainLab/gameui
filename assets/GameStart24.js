...
function CreateGameStart() {
    ...
    gameScoreTxt.text = "0";
    gameTimerTxt.scaleX = gameTimerTxt.scaleY = 1.10;
...
this.countTime = function () {
    ...
    gameTimerTxt.text = text
     ...
     createjs.Tween.get(gameTimerTxt).wait(200).to({scaleX:1.10,scaleY:1.10}, 500, 
        createjs.Ease.bounceOut).to({scaleX:0.8,scaleY:0.8}, 500, createjs.Ease.bounceIn); 
	...
	if(time == 8){
            ...
            createjs.Tween.get(questionOverImg).wait(100)
            .to({ alpha: 1, x: 0, y: 0 }, 8000)
        } 
