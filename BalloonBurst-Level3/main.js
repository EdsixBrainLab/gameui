...
var objArr0 = [{ x: 271.2 - 300, y: 143.4 }, { x: 119.2 - 300, y: 262.4 }, { x: 145.5 - 300, y: 280.6 }]

var objArr1 = [{ x: 329.1 - 230, y: 146.1 }, { x: 294.1 - 230, y: 269.1 }, { x: 350.1 - 230, y: 280.6 }]

var objArr2 = [{ x: 410.4 - 150, y: 140.2 }, { x: 437.4 - 150, y: 256.1 }, { x:513.9 - 150, y: 280.6 }]

var objArr3 = [{ x: 523.3 - 60, y: 122.7 }, { x: 569.3 - 60, y: 233.7 }, { x: 690.3 - 60, y: 280.6 }]

var objArr4 = [{ x: 271.2 - 300, y: 243.4 }, { x: 119.2 - 300, y:362.4 }, { x: 145.5 - 300, y: 458.6 }]

var objArr5 = [{ x: 329.1 - 230, y: 246.1 }, { x: 294.1 - 230, y: 369.1 }, { x: 350.1 - 230, y: 458.6 }]

var objArr6 = [{ x: 410.4 - 150, y: 240.2 }, { x: 437.4 - 150, y: 356.1 }, { x: 513.9 - 150, y: 458.6 }]

var objArr7 = [{ x: 523.3 - 60, y: 222.7 }, { x: 569.3 - 60, y: 333.7 }, { x: 690.3 - 60, y: 458.6 }]
...
//------------------------------------------------------------------------------------------------------------------//
var cnt = -1, qscnt = -1, ans, uans, interval, time = 180, totalQuestions = 10, answeredQuestions = 0, choiceCnt = 8, quesCnt = 0, resTimerOut = 0, rst = 0, responseTime = 0;
...
function init() {
    ...
    assetsPath = "assets/";
    gameAssetsPath = "BalloonBurst-Level3/";
    soundpath = "FA/"
    ...
    if (success == 1) {
        manifest.push(
            ...
            { id: "questiontext1", src: questionTextPath + "BalloonBurst-Level3-QT1.png" },
            { id: "questionText", src: questionTextPath + "BalloonBurst-Level3-QT.png" },
...
function doneLoading1(event) {
    ...
    if (id == "questionText") {
        var spriteSheet2 = new createjs.SpriteSheet({
            ...
            "frames": { "regX": 50, "height": 84, "count": 0, "regY": 50, "width": 780 },
            // define two animations, run (loops, 1.5x speed) and jump (returns to run):
...
function CreateGameElements() {
    ...
    questiontext1.visible = false;
    container.parent.addChild(questiontext1);
    questiontext1.x = questiontext1.x - 50;
    questionText.visible = false;
    container.parent.addChild(questionText);
    questionText.x = 280; questionText.y = 130;
    ...
    for (i = 0; i < choiceCnt; i++) {
        ...
        quesArr[i].x = 359;
        quesArr[i].y = 150;
        quesArr[i].scaleX = quesArr[i].scaleY = .85
    }
    ...
    choiceArr[0].x = 135; choiceArr[0].y = 350;
    choiceArr[1].x = 1010; choiceArr[1].y = 350;
    choiceArr[2].x = 430; choiceArr[2].y = 350;
...
function createChoices() {
    ...
    for (i = 0; i < 8; i++) {
        ...
        quesArr[i].x = 20 + (i * 100)
        quesArr[i].y = 150;
        // tweenMcArr[i].timeline.addTween(
        //     createjs.Tween.get(quesArr[i])
        //         .to({ x: 550, y: 50 }, 9).to({ x: this["objArr" + i][0].x + 250, y: this["objArr" + i][0].y + 20 }, timeFrameSpeed).to({ x: this["objArr" + i][1].x + 320, y: this["objArr" + i][1].y + 20 }, timeFrameSpeed).to({ x: this["objArr" + i][2].x + 320, y: this["objArr" + i][2].y + 70 }, timeFrameSpeed));
        tweenMcArr[i].timeline.addTween(
            createjs.Tween.get(quesArr[i])
                .to({ x: 550, y: 50 }, 9).to({ x: this["objArr" + i][0].x + 250, y: this["objArr" + i][0].y + 20 }, timeFrameSpeed)
                .to({ x: this["objArr" + i][1].x + 320, y: this["objArr" + i][1].y + 20 }, timeFrameSpeed)
                .to({ x: this["objArr" + i][2].x + 360, y: this["objArr" + i][2].y + 70 }, timeFrameSpeed));
